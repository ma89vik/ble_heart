...
static void i2s_ws2812b_drive_handler(nrf_drv_i2s_buffers_t const * p_released,
                         uint32_t  status)
{
    if (p_released->p_tx_buffer != NULL) {
      if (i2s_ws2812b_drive_flag_buffer_cnt == 0) {
        ...
        for (int i=0;i<(I2S_WS2812B_DRIVE_BUF_SIZE_PER_LED / 4);i++)
        {
          //p_released->p_tx_buffer[i] = 0;
